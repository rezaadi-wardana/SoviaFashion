...
const statusConfig: Record<string, { label: string; icon: any; color: string }> = {
  ...
  CANCELLED: { label: "Dibatalkan", icon: XCircle, color: "bg-red-100 text-red-700" },
}
      ...
      <div className="space-y-4">
        {loading ? (
          ...
          orders.map((order) => {
            ...
            return (
              <div key={order.id} className="bg-white rounded-lg p-6 shadow-sm border border-stone-100">
                <div className="flex flex-col md:flex-row justify-between items-start mb-6 gap-4 border-b border-stone-100 pb-4">
                  <div>
                    <p className="text-stone-500 text-sm mb-1">Order #{order.id.slice(-8)}</p>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-6">
                  {/* Items List */}
                  <div>
                    <h3 className="text-stone-900 font-medium mb-3 text-sm">Produk yang Dipesan</h3>
                    <div className="space-y-3">
                      {order.items.map((item) => {
                        ...
                        return (
                          <div key={item.id} className="flex items-center gap-3 bg-stone-50 p-2 rounded">
                            ...
                            <div className="flex-1">
                              <p className="text-stone-900 text-sm font-medium line-clamp-1">{item.product.name}</p>
                  ...
                  {/* Shipping Info */}
                  <div>
                    <h3 className="text-stone-900 font-medium mb-3 text-sm">Detail Pengiriman</h3>
                    <div className="bg-stone-50 p-4 rounded text-sm space-y-2">
                      <p><span className="text-stone-500">Penerima:</span> <span className="text-stone-900 font-medium">{order.recipientName}</span></p>
                      <p><span className="text-stone-500">No. HP:</span> <span className="text-stone-900">{order.phone}</span></p>
                      <p><span className="text-stone-500">Alamat:</span> <span className="text-stone-900 block mt-1">{order.address}</span></p>
                      {order.lat && order.lng && (
                        <a
                      ...
                      {order.trackingNumber && (
                        <p>
                          <span className="text-stone-500">No. Resi:</span>{' '}
                          <span className="text-stone-900 font-medium tracking-wide bg-stone-200 px-2 py-0.5 rounded">{order.trackingNumber}</span>
                        </p>
                      )}
                    </div>
                  </div>
                ...
                <div className="border-t border-stone-100 pt-4">
                  <h3 className="text-stone-900 font-medium mb-3 text-sm">Update Status Pesanan</h3>
                  {(order.status === "PACKING" || order.status === "PENDING_PAYMENT") && !order.isConfirmed ? (
                    <div className="flex gap-2">
                      ...
                      <button
                        ...
                        className="px-4 py-2 bg-red-100 text-red-600 rounded-lg text-xs font-medium hover:bg-red-200 transition-colors"
                      >
                        Tolak Pesanan
                      </button>
                    </div>
                  ) : order.status === "CANCELLED" ? (
                    <p className="text-red-500 text-sm font-medium">Pesanan ini telah dibatalkan.</p>
                  ) : (
                    <>
                            ...
                            <button
                              ...
                              className={`px-4 py-2 rounded-lg text-xs font-medium transition-colors border ${
                                isCurrent
                                  ...
                                  : isNext
                                  ? "bg-white text-stone-700 border-stone-300 hover:bg-stone-50 hover:border-stone-400 cursor-pointer shadow-sm"
                                  : "bg-white text-stone-400 border-stone-200 cursor-not-allowed opacity-60"
                              }`}
                            >
